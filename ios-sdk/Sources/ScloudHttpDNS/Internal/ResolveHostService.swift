...
final class ResolveHostService {
    ...
    private func isCacheEnabled() -> Bool {
        return getConfig().enableCacheIp
    }
    ...
    func getHttpDnsResultForHostSync(_ hostList: [String], requestIpType: ScloudRequestIpType) -> [ScloudHTTPDNSResult] {
        ...
        let now = nowMillis()
        let cacheEnabled = isCacheEnabled()
        var resultMap: [String: ScloudHTTPDNSResult] = [:]
        var pendingHosts: [String] = []

        for host in targets {
            if let cached = (cacheEnabled ? cache.get(host: host, type: requestIpType) : nil), (!cached.isExpired(nowMillis: now) || getConfig().enableExpiredIp) {
        ...
        if !pendingHosts.isEmpty {
            do {
                if pendingHosts.count == 1 || !cacheEnabled {
    ...
    func getHttpDnsResultForHostSync(_ host: String, requestIpType: ScloudRequestIpType) -> ScloudHTTPDNSResult {
        ...
        let now = nowMillis()
        if let cached = (isCacheEnabled() ? cache.get(host: normalizedHost, type: requestIpType) : nil), (!cached.isExpired(nowMillis: now) || getConfig().enableExpiredIp) {
            if cached.isExpired(nowMillis: now) {
